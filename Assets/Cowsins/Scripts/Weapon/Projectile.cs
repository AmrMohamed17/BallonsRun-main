            ...
            if (explosion)
            {
                ...
                if (hit.Length != 0)
                {
                    foreach (var h in hit)
                    {
                        ...
                        {
                            h.GetComponent<IDamageable>().Damage(explosionDamage / Vector3.Distance(h.transform.position, transform.position));
                        }
                        if (h.GetComponent<Rigidbody2D>() != null)
                        {
                            h.GetComponent<Rigidbody2D>().AddForce((h.transform.position - transform.position) / Vector3.Distance(h.transform.position, transform.position) * explosionForce * Time.deltaTime, ForceMode2D.Impulse);
                        }
                    }
                }
            }
